...
OpenTok.prototype.generateToken = function(sessionId, opts) {
  ...
  // TODO: validate that opts.initialLayoutClassList is a space separated string
  ...
  }), 'ist', 'iss', 'sub', 'exp', 'jti', 'role', 'scope', 'connectionData', 'initialLayoutClassList');
  ...
  if (!_.isNumber(tokenData.exp)) {
    throw new Error('Invalid expireTime for token generation: ' + tokenData.exp);
  }
  if (tokenData.exp < now) {
    throw new Error('Invalid expireTime for token generation, time cannot be in the past: ' +
                    tokenData.exp + ' < ' + now);
  }
  if (tokenData.connectionData &&
      (tokenData.connectionData.length > 1024 || !_.isString(tokenData.connectionData))) {
    throw new Error('Invalid data for token generation, must be a string with maximum length 1024');
  }

  // TODO: validate that opts.initialLayoutClassList length is less than maxiumum

  return jwt.encode(tokenData, this.apiSecret);
};
